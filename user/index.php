...
if(isset($_POST['login'])){
    ...
    $sql=$connect->query("select * from users where user_name='$usernm'");
    if(mysqli_num_rows($sql)>0){
      $data=mysqli_fetch_array($sql);
      if($passwrd==$data['user_pass']){
     ...
     <div class="container mt-5 w-25 shadow p-4 rounded">
        <form method="post" action="#" class="d-flex flex-column" novalidate>
          ...
          <div class="mt-3">
            ...
            <button class="btn btn-secondary w-100" type="button" name="login" onclick="window.location.href='create_account.php' ">Sign Up</button>
          </div>
        </form>
    </div>
